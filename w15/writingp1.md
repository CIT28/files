...
#### Setup and Foreign Key Constraint Error
- Delete the `mfa.db`, create a new file named schema.sql copy contents from here [schema](#MFA)
- Create the `mfa.db` and then run `.read schema.sql`in sqlite3 
- In your p1-q.sql file write our "normal" queries for each table in the newly created `mfa.db`.
- Add `.schema` to verify the new mfa database and output `pow-p1.txt`.
- In sqlite3 terminal run this command:
  - `PRAGMA foreign_keys = ON;`  
- Now write the SQL to delete the artist with Carter.  
- Observe the FOREIGN KEY constraint error.
  - If it did delete and you don't get the error you missed running the command above.
- Add a `.print` describing what happened and why the error occurred.
- Add notes regarding why this error occurs, select your own heading.
- Write the subquery to first delete the artist's affiliation from `created`
- Then write SQL delete the artist from `artists`.
- Add notes to explain the and use heading two-step delete process
- Verify it was successful 
- Output all table data (uncomment) above to `pow-p1.txt`.

💾 **Commit Message:** `"foreign key constraint error when deleting artist w/2 steps process"`
